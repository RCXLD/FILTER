...
int CCheckAcl::SendSessionToApp(SESSION *session)
{
	...
	if(m_GuiHwnd != NULL && _tcscmp(m_AclFile.mAclHeader.sSignature, XF_INVALID_PROCESS) != 0)
	{
		int		iIndex;
...
int CCheckAcl::CheckSend(SOCKET s, TCHAR *buf, int len, LPDWORD lpNumberOfBytesSent)
{
	...
	CProtocolInfo::GetProtocolInfo(m_Session + iIndex, buf, len, TRUE);
...
int CCheckAcl::CheckRecv(SOCKET s, TCHAR *buf, int len, LPDWORD lpNumberOfBytesRecvd)
{
	...
	CProtocolInfo::GetProtocolInfo(m_Session + iIndex, buf, len, FALSE);
